...
# Plot Baseline
g_base = df_merged[df_merged["method"] == "Baseline"].sort_values("batch_size")

for idx, algo in enumerate(ALGORITHMS):
    ax = fig.add_subplot(2, 2, idx + 1, projection='3d')
    g_algo = df_merged[df_merged["method"] == algo]
    pivot = g_algo.pivot_table(index="batch_size", columns="total_params_millions", values="peak_inference_memory_mb")
    pivot = pivot.sort_index(axis=1)
    
    pivot = pivot.interpolate(axis=1).bfill(axis=1).ffill(axis=1)
    
    X_mesh, Y_mesh = np.meshgrid(np.log2(pivot.index.values), pivot.columns.values)
    Z = pivot.values.T
    
    color = ALGO_COLORS[algo]
    surf = ax.plot_surface(X_mesh, Y_mesh, Z, color=color, alpha=0.8, edgecolor='black', linewidth=0.3)
    
    # Plot Baseline
    if not g_base.empty:
        X_base = np.log2(g_base["batch_size"].values)
        Y_base = np.full_like(X_base, baseline_params / 1e6)
        Z_base = g_base["peak_inference_memory_mb"].values
        ax.plot(X_base, Y_base, Z_base, color="black", linewidth=5, label="Baseline", zorder=10)
    
    ax.set_title(f"Algorithm: {algo}", fontsize=16, fontweight="bold", color=color)
    ax.set_xlabel("Log2(Batch Size)", fontsize=12, labelpad=10)
...
plt.suptitle("3D Peak Memory Scaling: Batch Size vs Parameters", fontsize=22, fontweight="bold", y=0.95)
plt.tight_layout(rect=[0, 0, 1, 0.93])

out_path = os.path.join(PLOT_DIR, "batch_memory_3d_combined.png")
plt.savefig(out_path, dpi=300, bbox_inches="tight")
plt.close()

print(f"Saved 3D combined plot to {out_path}")
